...
{
    class ActorProxyTypeFactory
    {
        /// <summary>
        /// Creates a new instance of the <see cref="ActorProxyTypeFactory"/> class.
        /// </summary>
        /// <param name="target"></param>
        public ActorProxyTypeFactory(TypeInfo target)
        {
            this.target = target;
        }
        ...
        {
            return (T)new ActorProxyTypeFactory(typeof(T).GetTypeInfo()).Create(new object[] { deactivation }.Concat(args).ToArray());
        }
        public static TypeInfo CreateType<T>()
        {
            return new ActorProxyTypeFactory(typeof(T).GetTypeInfo()).CreateType();
        }
        /// <summary>
        /// Creates the proxy registered with specific interceptor.
        /// </summary>
        ...
        public object Create(object[] args)
        {
            ...
            TypeInfo proxy = this.typeBuilder.CreateTypeInfo();
            
            return Activator.CreateInstance(proxy.GetType(), args);
        }
        ...
        public void BuildType()
        {
            ...
            this.typeBuilder =
                this.moduleBuilder.DefineType(target.FullName + "Proxy", TypeAttributes.Class | TypeAttributes.Public, target.GetType());
        ...
        public void InterceptAllMethods()
        {
            ...
            var methodInfo = target.GetMethod("OnDeactivateAsync", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.FlattenHierarchy);
            {
                if (methodInfo.IsVirtual)
                {
                    ...
                    ilGenerator.Emit(OpCodes.Call, typeof(IActorDeactivationInterception).GetTypeInfo().GetMethod("Intercept"));
